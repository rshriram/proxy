...
void Filter::cacheFilterMetadata(
  ...
  for (auto &filter_pair : filter_metadata) {
    if ((filter_pair.first ==
        ...
        (filter_pair.first ==
         Extensions::NetworkFilters::NetworkFilterNames::get().MySQLProxy)) {
      ...
      for (const auto &message_pair : filter_pair.second.fields()) {
        cached_fields[message_pair.first].mutable_list_value()->CopyFrom(message_pair.second.list_value());
      }
    }
  }
}
